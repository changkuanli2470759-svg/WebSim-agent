...
def api_session_state():
    """Restore a persisted browser session without resetting recommendation history."""
    include_history_comments = parse_bool(request.args.get("include_history_comments"), True)
    ...
    page_idx = max(0, int(session.get("page_idx", 0)))
    cards = enrich_cards(paginate_cards(recommender, rec_ids, page_idx), track_impressions=True)
    cards = attach_comments_to_cards(cards, dataset_key=dataset_key, include_history_comments=include_history_comments)
    return jsonify({
        "has_session": True,
        "cards": cards,
        "mode": model_name,
        ...
        "available_models": recommender.available_models(),
        "page": page_idx + 1,
        "has_next": len(rec_ids) > (page_idx + 1) * PAGE_SIZE,
