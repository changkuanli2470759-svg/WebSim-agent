...
async function restorePage() {
  const data = await fetchJSON(
    `/api/session_state?dataset_key=${encodeURIComponent(datasetSelect.value)}&include_history_comments=${historyCommentsEnabled() ? "1" : "0"}`
  );
  if (!data.has_session) {
    return initPage();
  }
  ...
  statusText.textContent = `${data.dataset_label || datasetLabel(data.dataset_key)} / ${modelLabel(data.model_name)} restored at page ${data.page}.`;
}
...
const resumeSession = new URLSearchParams(window.location.search).get("resume") === "1";
(resumeSession ? restorePage() : initPage()).catch((err) => {
